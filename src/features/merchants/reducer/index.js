        ...
        switch (type) {
            ...
            case types.merged:
                if (
                    ...
                    console.log("reducer work", payload?.merchants)
            ...
            case types.statusWebsiteUpdated:

                if (draft.all[payload.merchantId]) {
                    const websiteIndex = draft.all[payload.merchantId].websites.findIndex(
                        (site) => site.id === payload.websiteId
                    );
                    if (websiteIndex !== -1) {
                        draft.all[payload.merchantId].websites[websiteIndex].status = payload.status;
                        console.log("statusWebsiteUpdated", payload.status)
                    }
                }
                break;

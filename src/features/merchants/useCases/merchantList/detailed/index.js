...
import actions from 'features/merchants/actions';

import useDetailedMerchant from './service'
...
function ActionMenu({ anchorEl, isMenuOpen, handleMenuClose, handleOpen, merchantId, websiteId }) {
    console.log("website id :", websiteId)
    const dispatch = useDispatch();

    const handleOpenReject = React.useCallback(() => {
        dispatch(actions.changeWebsiteStatus({ merchantId, websiteId, status: 'Invalid' }));// later add reason
        handleMenuClose();
    }, [dispatch, merchantId, handleMenuClose]);

    const handleApprove = React.useCallback(() => {
        dispatch(actions.changeWebsiteStatus({ merchantId, websiteId, status: 'Verified' }));
        handleMenuClose();
    }, [dispatch, merchantId, handleMenuClose]);
        ...
        <>
            ...
            <Grid container style={{ border: 1, borderColor: 'gray', marginLeft: 50, marginRight: 50, padding: 20 }}>
                ...
                <Grid item container xs={12} md={4} style={{ height: '340px' }}>
                    ...
                    {merchant?.websites.map((website, index) => (
            ...
            <DeleteModal open={modal.openDelete} handleCancel={handleModalClose} id={merchant?.id} pageName="detailed" />
            <ChangeStatusModal open={modal.openDeacivate} handleCancel={handleModalClose} id={merchant?.id} />
        </>
    );
}

...
const MerchantCard = ({ merchant, handleOpen }) => {
    const { id, firstName, lastName, status, balance, websites } = merchant;
    ...
    const handleGotoDetails = React.useCallback(() => {
        ...
        navigate('/adminDashboard/buyers/detailed')
    }, [navigate])

    const handleActivitiStatusChnage = useCallback(
        e => {
            if (statusValue === "active") handleDeactivate({ type: 'Inactive', id })
            else dispatch(actions.changeStatus({ merchantId: id, status: 'active' }))
        },
        [statusValue]
    )

...
export default function useDetailedclient() {
    ...
    const user = useSelector(detailedSelected);
    ...
    const handleActivitiStatusChnage = useCallback(
        e => {
            if (user?.status[0]?.value === "active") handleModalOpen({ type: 'deactivate' })
            else dispatch(actions.changeStatus({ userId: user?.id, status: 'active' }))
        },
        [handleModalOpen, user, dispatch]
    )
